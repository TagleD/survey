...
function updateMaxInputValue(answerGroup) {
    const formInputs = document.querySelectorAll('[id^="formInnputID_"]');
    const maxInputValue = answerGroup.length;
    formInputs.forEach(input => {
        input.max = maxInputValue;
...
function createCheckboxForm() {
    ...
    numberOfCheckboxesInput.id = `formInnputID_${formCounter}`;
    numberOfCheckboxesInput.min = 1;
    numberOfCheckboxesInput.max = answerGroup.length + 1;
    numberOfCheckboxesInput.value = 1;
    ...
    const addButton = createSvgButton(
        ...
        (event) => {
            ...
            updateMaxInputValue(answerGroup);
        });
...
function createCheckboxAnswerOption(parentContainer, checked, formCount, newForm) {
    ...
    radioButton.name = `checkboxQuestion_${formCount}`;
